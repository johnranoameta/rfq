...
- **Header**: `WorkbookHeader` has no `program` field of its own — `workbookToAgentParsed.ts` computes the final `program` value as `[header.region, header.sop].filter(Boolean).join(" · ") || header.rfq_id`. So: `customer: ""`, `region: ""`, `sop: ""` (all blank — none of these exist in this shape), `rfq_id:` the first non-empty `customer_program` value found across `parts` rows (best-effort label, not a real customer field — this shape has none). With region/sop blank, `workbookToAgentParsed`'s existing fallback surfaces this value as `program` without any change to that file. `annual_volume: 0`, `currency: "USD"`.
